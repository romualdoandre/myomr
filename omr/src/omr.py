...
class Omr:
    ...
    def scan(self):
        ...
        if self.inputfolder!=None and self.appfilepath!=None and self.datafilepath!=None and self.svmfile!=None:
            self.core=Core(glob.glob(self.inputfolder+'/*.'+self.extension),self.appfilepath,self.datafilepath,self.svmfile)
            self.core.run()
        else:
...
if __name__ == "__main__":
    ...
    parser.add_option("-l", "--learn",action="store", type="string", dest="svmfile",help='file with the result from a trained SVM')
