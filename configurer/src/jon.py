...
class Jon():
    """
    Módulo do sistema de correções via computação visual. Jon é utilizado para criar um arquivo de configuração para o sistema Arya.
    ...
    def make_menu(self):
        """
        Criação do menu da janela principal
        """
        top=Menu(self.root)
        file = Menu(top,tearoff=False)
        file.add_command(label='Novo',  command=self.new_app,  underline=0,accelerator="Ctrl+n")
        file.add_command(label='Abrir Imagem', command=self.set_photo,  underline=6,accelerator="Ctrl+i")
        file.add_command(label='Abrir Aplicação', command=self.load_app,  underline=0,accelerator="Ctrl+a")
        file.add_command(label='Salvar Aplicação',  command=self.save_app,  underline=0,accelerator="Ctrl+s")
        file.add_separator()
        file.add_command(label='Sair',    command=self.root.quit, underline=0,accelerator="Ctrl+q")
        top.add_cascade(label='Arquivo',     menu=file,        underline=0)
        edit= Menu(top, tearoff=False)
        edit.add_command(label='Novo',     command=self.menu_edit_new,  underline=0)
        edit.add_command(label='Mover',   command=self.menu_edit_move,  underline=0)
        top.add_cascade(label='Campo',     menu=edit,        underline=1)
        help= Menu(top, tearoff=False)
        help.add_command(label='Sobre',     command=self.about,  underline=0)
        top.add_cascade(label='Ajuda',     menu=help,        underline=1)
    ...
    def new_app(self):
        """
        Criação de uma nova aplicação (configuração para o Arya)
        """
        self.field={}
        self.fields={}
        self.tag=0
    ...
    def about(self):
        """
        Mostra uma caixa de diálogo com a mensagem sobre o Jon.
        """
        tkMessageBox.showinfo("Sobre", "Jon 1.0\nLicenciado sobre a GPL v3")

    def select(self,event):
        """ 
        Seleção da região ou campo de uma prova já estabelecido.
    ...
    def config_item(self,event):
        """
        Configuração de um campo de uma prova.
        @param event Event: evento do mouse
        """
        self.canvas.itemconfigure('region', outline='black')
        self.canvas.itemconfigure('tag', fill='black')
    ...
    def xy(self,event):
        """
        Determinação do x e y iniciais da região ou campo da prova
    ...
    def add_rectangle(self,event):
        """
        Demarcação de uma região ou campo de uma prova com um retângulo
        @param event Event: evento do mouse
        """
        if self.objeto:
    ...
    def load_fields(self):
        """
        Carrega os campos do arquivo de máscara de correções
        """
